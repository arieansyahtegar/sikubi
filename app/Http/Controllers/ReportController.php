...
class ReportController extends Controller
{
    ...
    public function printRecap(Request $request)
    {
        ...
        $transactions = $query->get()->map(function ($tx, $index) {
            return [
                ...
                'account'     => $tx->bankAccount->account_alias ?? $tx->bankAccount->bank_name ?? 'Transaksi Tunai',
            ];
        });
    ...
    public function recapCsv(Request $request)
    {
        ...
        if ($accountId) {
            if ($accountId === 'cash') {
                $accountSummaryQuery->whereNull('bank_account_id');
            } else {
        ...
        // Also add manual cash account in recap if applicable
        if (!$accountId || $accountId === 'cash') {
            $cashTotals = $accountTotals->get(null, collect());
            $debit = $cashTotals->where('type', 'DEBIT')->first()->total ?? 0;
            $credit = $cashTotals->where('type', 'CREDIT')->first()->total ?? 0;
            fputcsv($out, ['Manual', 'Transaksi Tunai', $debit, $credit, $debit - $credit]);
        }
        ...
        if ($accountId) {
            if ($accountId === 'cash') {
                $categorySummaryQuery->whereNull('bank_account_id');
            } else {
        ...
        if ($accountId) {
            if ($accountId === 'cash') {
                $txQuery->whereNull('bank_account_id');
            } else {
        ...
        $txQuery->chunk(500, function ($txs) use ($out) {
            foreach ($txs as $tx) {
                fputcsv($out, [
                    ...
                    $tx->type,
                    $tx->bankAccount->account_alias ?? $tx->bankAccount->bank_name ?? 'Transaksi Tunai',
                    $tx->amount
                ]);
            }
        });
    ...
    public function recapExcel(Request $request)
    {
        ...
        if ($accountId) {
            ...
            $accounts = BankAccount::orderBy('id', 'asc')->get()->map(fn($item) => (object)$item->toArray());
        ...
        if ($accounts->isEmpty()) {
            ...
            foreach ($accounts as $acc) {
                ...
                $inBefore = Transaction::where('transaction_date', '<', $startDate);
                $outBefore = Transaction::where('transaction_date', '<', $startDate);
                if (is_null($acc->id)) {
                    $inBefore->whereNull('bank_account_id');
                    $outBefore->whereNull('bank_account_id');
                } else {
                    $inBefore->where('bank_account_id', $acc->id);
                    $outBefore->where('bank_account_id', $acc->id);
                }
                ...
                if (is_null($acc->id)) {
                    ...
                    $query->where('bank_account_id', $acc->id);
                }
