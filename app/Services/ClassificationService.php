...
class ClassificationService
{
    ...
    public function classify(string $description, string $type, ?int $accountId = null): array
    {
        ...
        // Stage 1: Rule-based matching (exact/contains/regex from DB)
        ...
        // Stage 1.5: Direct robust keyword classification fallback (account-aware)
        $categoriesQuery = \App\Models\Category::query();
        if ($accountId) {
            $categoriesQuery->where(function ($q) use ($accountId) {
                $q->where('bank_account_id', $accountId)
                  ->orWhereNull('bank_account_id');
            });
        } else {
            $categoriesQuery->whereNull('bank_account_id');
        }
        ...
        if ($type === 'DEBIT') {
            ...
            } elseif (str_contains($upperDesc, 'TF') || str_contains($upperDesc, 'TRANSFER') || str_contains($upperDesc, 'TRSF') || str_contains($upperDesc, 'MASUK') || str_contains($upperDesc, 'BI-FAST') || str_contains($upperDesc, 'BI FAST') || str_contains($upperDesc, 'FAST') || str_contains($upperDesc, 'SWITCHING') || str_contains($upperDesc, 'KLIRING') || preg_match('/\bCR\b/', $upperDesc) || str_contains($upperDesc, 'MUTASI')) {
    ...
    private function matchesRule(string $upperDesc, $rule): bool
    {
        ...
        return match ($rule->match_type) {
            ...
            'REGEX' => (@preg_match('/' . $pattern . '/i', $upperDesc) === 1),
            default => false,
        };
    }
