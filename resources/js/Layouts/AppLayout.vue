    ...
    <div class="flex h-screen overflow-hidden bg-cream">
        ...
        <aside
            :class="[
                ...
                isSidebarCollapsed ? 'w-[272px] lg:w-20' : 'w-[272px]'
            ]"
        >
            <!-- Logo -->
            <div class="flex items-center gap-3 px-6 py-5 border-b border-rose-100/60" :class="isSidebarCollapsed ? 'lg:justify-center lg:!px-4' : ''">
            ...
            <!-- Navigation -->
            <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto" :class="isSidebarCollapsed ? 'lg:!px-2' : ''">
                <p :class="['px-4 pt-2 pb-2 text-[10px] font-bold text-surface-500 uppercase tracking-widest', isSidebarCollapsed ? 'lg:hidden' : '']">Menu Utama</p>
                <Link
                ...
                <p :class="['px-4 pt-5 pb-2 text-[10px] font-bold text-surface-500 uppercase tracking-widest', isSidebarCollapsed ? 'lg:hidden' : '']">Pengaturan</p>
                <Link
            ...
            <!-- User Profile -->
            <div class="px-4 py-4 border-t border-rose-100/60" :class="isSidebarCollapsed ? 'lg:!px-2' : ''">
                <div class="flex items-center gap-3 p-3 rounded-xl bg-cream-200/50 hover:bg-cream-300/50 transition-colors" :class="isSidebarCollapsed ? 'lg:justify-center lg:!p-2' : ''">
                    ...
                    <Link :class="['flex-1 min-w-0 cursor-pointer', isSidebarCollapsed ? 'lg:hidden' : '']" href="/profile" @click="sidebarOpen = false">
                    ...
                    <Link
                        :class="['p-1.5 text-surface-500 hover:text-red-500 rounded-lg transition-colors', isSidebarCollapsed ? 'lg:hidden' : '']"
                        href="/logout"
                        method="post"
                        as="button"
                        title="Keluar"
                                    ...
                                    <div v-for="item in visibleNotifications" :key="item.id" class="w-full text-left px-4 py-3 hover:bg-cream-100 transition-colors border-b border-rose-50/60 last:border-0 relative group cursor-pointer" @click="goToNotif(item)">
                                        ...
                                        <div class="flex items-start gap-3 pr-6">
                                            <div :class="['w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 text-sm',
                                                ...
                                                (item.type === 'leader_action' || item.type === 'leader_decision') ? 'bg-violet-100 text-violet-600' :
                                                item.type === 'anomaly_decided' ? 'bg-blue-100 text-blue-600' :
                                                'bg-blue-100 text-blue-600'
                                            ]">
                                                <svg v-if="item.type === 'anomaly' || item.type === 'anomaly_decided'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>
                                                ...
                                                <svg v-else-if="item.type === 'leader_action' || item.type === 'leader_decision'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /></svg>
