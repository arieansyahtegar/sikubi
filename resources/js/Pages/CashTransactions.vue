            ...
            <Transition name="slide-up">
                <div v-if="showForm && canManage" class="glass-card p-6">
                    <div class="flex items-center gap-2.5 mb-4 pb-3 border-b border-rose-100/40">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-plum">Tambah Transaksi Tunai Baru</h3>
                            <p class="text-xs text-surface-500">Catat pemasukan atau pengeluaran kas tunai secara manual</p>
                        </div>
                    </div>
                    <form @submit.prevent="submit" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="sm:col-span-2 lg:col-span-3 flex justify-end gap-2 pt-3 mt-1 border-t border-rose-100/30">
                            ...
                            <button type="submit" :disabled="form.processing" class="btn-primary text-xs">
                                <svg v-if="form.processing" class="w-4 h-4 animate-spin text-white mr-1" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                                Simpan Transaksi
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="sm:hidden space-y-3">
                        <div v-for="tx in transactions.data" :key="tx.id" class="mobile-card space-y-2.5">
                            ...
                            <div class="flex justify-between items-center text-xs text-surface-500">
                                ...
                                <span v-if="tx.category" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold" :style="{ background: tx.category.color + '12', color: tx.category.color, border: '1px solid ' + tx.category.color + '30' }">
                                    {{ tx.category.name }}
                                </span>
                            </div>
